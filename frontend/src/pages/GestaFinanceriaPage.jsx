...
export default function GestaFinanceiraPage() {
    ...
    const features = [
        { icon: "📋", titulo: "Contas a pagar",        desc: "Cadastre títulos manualmente ou importe via Excel. Controle vencimentos, saldos e status em tempo real.",         badge: "Disponível",   badgeColor: "#4ADE80" },
        { icon: "📊", titulo: "Relatórios financeiros", desc: "Visão consolidada por período, fornecedor e status. Exporte para Excel ou PDF com um clique.",                   badge: "Em breve",     badgeColor: "#60A5FA" },
        { icon: "🔔", titulo: "Alertas de vencimento",  desc: "Receba e-mails automáticos dias antes do vencimento. Configure a antecedência conforme sua necessidade.",       badge: "Em breve",     badgeColor: "#60A5FA" },
        { icon: "🏦", titulo: "Remessa CNAB integrada", desc: "Gere arquivos de remessa bancária diretamente dos títulos cadastrados — sem redigitar dados.",                   badge: "Em breve",     badgeColor: "#60A5FA" },
        { icon: "📥", titulo: "Importação via Excel",   desc: "Compatível com exportações do Protheus (SE2/E2) e planilhas customizadas. Importe centenas de títulos.",        badge: "Disponível",   badgeColor: "#4ADE80" },
        { icon: "🔄", titulo: "Integração Protheus",    desc: "Busque títulos diretamente do ERP, gere remessas e faça write-back de baixas automaticamente.",                 badge: "Sob consulta", badgeColor: "#A78BFA" },
    ];

    const planos = [
        { nome: "Gratuito", preco: "R$ 0",         cor: "var(--text-dim)", items: ["8/mês",    "Excel e PDF", "Todos", "—",  "—",  "—" ] },
        { nome: "Pro",      preco: "R$ 18,90/mês", cor: "#A78BFA",         items: ["Ilimitado","Excel e PDF", "Todos", "—",  "—",  "—" ] },
        { nome: "Whallet+", preco: "R$ 39,90/mês", cor: "#FCD34D",         items: ["Ilimitado","Excel e PDF", "Todos", "✓",  "✓",  "✓" ] },
    ];
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 64px" }}>
            ...
            <div style={{ textAlign: "center", marginBottom: 56 }}>
                ...
                <h1 style={{
                    ...
                    <span style={{ color: "#FCD34D" }}>ao seu workflow bancário</span>
                </h1>
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                    <button onClick={handleUpgrade} disabled={carregando} style={{
                        ...
                        border: "none", color: "#1a1a1a", fontWeight: 800, fontSize: 16,
                ...
                {erro && <div style={{ marginTop: 16, fontSize: 14, color: "#F87171" }}>{erro}</div>}

                <div style={{
                    display: "inline-flex", alignItems: "center", gap: 6, marginTop: 16,
                    background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.15)",
                    borderRadius: 20, padding: "4px 14px", fontSize: 12, color: "#FCD34D", fontWeight: 600
                            ...
                            <div key={p.nome} style={{ padding: "0 16px", borderLeft: i > 0 ? "1px solid var(--border)" : "none" }}>
                                ...
                                {linhas.map((label, j) => (
                                    <div key={label} style={{ padding: "8px 0", borderTop: "1px solid var(--border)", fontSize: 12 }}>
                                        ...
                                        <div style={{
                                            fontWeight: 600,
                                            color: p.items[j] === "✓" ? "#4ADE80" : p.items[j] === "—" ? "rgba(255,255,255,0.15)" : "var(--text)"
                                        }}>{p.items[j]}</div>
                                    </div>
                                ))}
                            </div>
            ...
            {/* ── CTA final ── */}
            <div style={{
                background: "linear-gradient(135deg, rgba(245,158,11,0.08), rgba(252,211,77,0.05))",
                border: "1px solid rgba(251,191,36,0.2)", borderRadius: 20, padding: "40px 32px", textAlign: "center"
            }}>
                ...
                {!autenticado && (
                    <p style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 12 }}>
                        Já tem conta? <Link to="/login" style={{ color: "#FCD34D" }}>Entrar</Link>
                    </p>
                )}
            </div>
        </div>
