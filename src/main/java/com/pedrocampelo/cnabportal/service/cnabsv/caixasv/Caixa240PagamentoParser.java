package com.pedrocampelo.cnabportal.service.cnabsv.caixasv;

import com.pedrocampelo.cnabportal.layout.BankLayoutField;
import com.pedrocampelo.cnabportal.layout.caixa.Caixa240PagamentoLayout;
...
/**
 * Parser para Caixa Econômica Federal — CNAB 240 Pagamentos (Remessa e Retorno).
 *
 ...
 */
@Service
public class Caixa240PagamentoParser implements CnabParser {
    ...
    public List<ParsedRecord> parse(MultipartFile file) throws IOException {
        ...
        if (headerArquivo.length() < 143) {
            throw new IllegalArgumentException(
                    "Arquivo CNAB 240 Caixa inválido: header de arquivo muito curto (" +
                            headerArquivo.length() + " bytes, esperado >= 143).");
        }

        char tipoArquivo = headerArquivo.charAt(142); // pos 143
        System.out.println("[Caixa240Pagamento] tipo=" +
        ...
        for (String line : lines) {
            ...
            String recordType = Caixa240PagamentoLayout.getRecordType(line);
            List<BankLayoutField> fields = Caixa240PagamentoLayout.getFieldsForLine(line);
