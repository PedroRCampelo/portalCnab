package com.pedrocampelo.cnabportal.service.cnabsv.bbsv;

import com.pedrocampelo.cnabportal.layout.BankLayoutField;
import com.pedrocampelo.cnabportal.layout.bb.BB240PagamentoLayout;
...
/**
 * Parser para Banco do Brasil CNAB 240 — Pagamentos (Remessa e Retorno).
 *
 ...
 */
@Service
public class BB240PagamentoParser implements CnabParser {
    ...
    public List<ParsedRecord> parse(MultipartFile file) throws IOException {
        ...
        if (headerArquivo.length() < 143) {
            throw new IllegalArgumentException(
                    "Arquivo CNAB 240 BB inválido: header de arquivo muito curto (" +
                            headerArquivo.length() + " bytes, esperado >= 143).");
        }

        char tipoArquivo = headerArquivo.charAt(142); // pos 143
        System.out.println("[BB240Pagamento] tipo=" +
        ...
        for (String line : lines) {
            ...
            String recordType = BB240PagamentoLayout.getRecordType(line);
            List<BankLayoutField> fields = BB240PagamentoLayout.getFieldsForLine(line);
